...
private:
    ...
    void update(int p, int L, int R, int i, int j, ll val) {
        ...
        if((L >= i) && (R <= j)) {
            ...
            int lsubtree = (lazy[l(p)] != -1) ? lazy[l(p)] : st[l(p)]; // change -1,0
            int rsubtree = (lazy[r(p)] != -1) ? lazy[r(p)] : st[r(p)]; // change -1,0
...
public:
    Segtree(int sz) : n(sz), st(4*n), lazy(4*n, -1) {} // change -1,0
...
int main() {
    ...
    int n, m, k, a, b; cin >> n >> m; ll x;
    vi A(n); FOR(i,0,n) cin >> A[i]; Segtree st(A);
    FOR(_,0,m) {
        cin >> k;
        if(k == 1) {cin >> a >> x; st.update(a, a, x);} 
        else {cin >> a >> b; cout << st.RMQ(a,b-1) << endl;}
    }
    return 0;
}